...
const UserPortal = () => {
  const { user, solicitudes, documentos, mensajes, logout, uploadDocument, sendMessage, markMessagesAsRead, updateSolicitudDescripcion, signalRConnection, getAccessToken } = useAuth();
  ...
  useEffect(() => {
    ...
    const mobileMediaQuery = globalWindow.matchMedia('(max-width: 1023px)');
    const compactHeaderMediaQuery = globalWindow.matchMedia('(max-width: 1279px)');

    const syncResponsiveState = () => {
      setIsMobile(mobileMediaQuery.matches);
      setIsCompactHeader(compactHeaderMediaQuery.matches);
    };

    syncResponsiveState();

    if (mobileMediaQuery.addEventListener && compactHeaderMediaQuery.addEventListener) {
      mobileMediaQuery.addEventListener('change', syncResponsiveState);
      compactHeaderMediaQuery.addEventListener('change', syncResponsiveState);
      return () => {
        mobileMediaQuery.removeEventListener('change', syncResponsiveState);
        compactHeaderMediaQuery.removeEventListener('change', syncResponsiveState);
      };
    }

    mobileMediaQuery.addListener(syncResponsiveState);
    compactHeaderMediaQuery.addListener(syncResponsiveState);
    return () => {
      mobileMediaQuery.removeListener(syncResponsiveState);
      compactHeaderMediaQuery.removeListener(syncResponsiveState);
    };
  }, []);
